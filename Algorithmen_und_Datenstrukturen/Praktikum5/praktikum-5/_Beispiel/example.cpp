...
//[example]
int level;				/* additional data structures */
void preDFS() {			/* actions directly before DFS() */
	cout << "n = " << n << ", m = " << m << endl;
	level = -1;
	cout << "(node,level):";
}
void dfsVisit(const int v) {	/* preorder actions at node v */
	cout << " (" << v << "," << ++level << ")";
}
void finVisit(const int v) {	/* postorder actions at node v */
	--level;
}
void postDFS() {		/* actions directly after DFS() */
	cout << endl << "dfsNum:";
	for(int i : dfsNum) cout << " " << i;
	cout << endl << "T:";
